# OB01 staff photo pilot

This is an isolated, upload-only staff mailbox for First Nation Smoke Cannabis Dispensary Toronto. It does not connect to GBP, Signal, another store, or a fleet database. The future central collector is responsible for validation, GBP posting, Signal morning messages, and incomplete reminders.

## Before enabling

1. Run `supabase/staff-photo-pilot.sql` manually in First Nation Smoke Cannabis Dispensary Toronto's own Supabase project. It creates a private bucket and private tables with no browser policies.
2. Configure these server-only deployment variables. Never prefix them with `NEXT_PUBLIC_`:
   - `OB_STAFF_SUPABASE_URL`
   - `OB_STAFF_SUPABASE_SERVICE_ROLE_KEY`
   - `OB_STAFF_DAILY_PIN_SECRET` (at least 32 random characters; separate from all other secrets)
   - `OB_STAFF_SESSION_SECRET` (at least 32 random characters)
   - `OB_STAFF_RETRIEVAL_TOKEN` (at least 32 random characters)
   - `OB_STAFF_CLEANUP_TOKEN` (optional separate token; retrieval token is fallback)
   - `CRON_SECRET` (at least 32 random characters; used only by Vercel Cron)
   - `OB_STAFF_PIN_BOUNDARY_HOUR` (optional, defaults to `6` in Toronto)
   - `OB_STAFF_PREVIOUS_PIN_GRACE_MINUTES` (optional, defaults to `0`; enabling grace eases shift handoff but delays removal from the prior Signal group)
3. Deploying `vercel.json` registers a once-daily cleanup on every Vercel plan. Vercel calls `GET /api/staff-photo/maintenance/cleanup` with `CRON_SECRET`. The collector may also trigger authenticated cleanup with `POST` and `OB_STAFF_CLEANUP_TOKEN`.
...
All collector routes require `Authorization: Bearer <OB_STAFF_RETRIEVAL_TOKEN>`.
