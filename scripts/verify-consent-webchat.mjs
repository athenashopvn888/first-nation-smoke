...
const chat = await readFile(new URL("../app/delivery/FirstNationSmokeWebChat.tsx", import.meta.url), "utf8");
...
assert.ok(delivery.includes("<FirstNationSmokeWebChat />"), "Delivery page must render First Nation Smoke Web Chat");
assert.ok(!chat.includes('storeId: "PC"') && !chat.includes("sod-web-chat:PC"), "Reference store identity must not remain");
console.log("First Nation Smoke consent Web Chat contract passed.");
