...
export function getStrainData(
  ...
  const description = `${name} is listed as ${tierDesc} in the ${typeLabel} lane at First Nation Smoke Cannabis Dispensary Toronto${thcLine}. Compare the current product page, menu details, weight, and posted price before choosing.`;

  const metaDescription = `${name} at First Nation Smoke Cannabis Dispensary Toronto in Toronto. Compare the current ${tierDesc} listing, current details, and staff guidance before visiting.`;
